...
export class FileSystem {
  ...
  /**
   * Generates a random hash key to prevent duplicate downloads
   *
   * @returns {string}
   */
  generateCacheHashKey = () => {
    let hash               = '';
  ...
  /**
   * Generates the full path to the download file
   *
   * @param path
   * @returns {`${string}.incomplete_${string}`}
   */
  getDownloadFilePathForPath = (path) => {
    const hash = this.generateCacheHashKey();

    return `${path}.incomplete_${hash}`;
  };
  ...
  async fetchFile(url, permanent = false, fileName = null, clobber = false) {
    ...
    let result = null;
    const incompleteFilePath = this.getDownloadFilePathForPath(path);
    ...
    try {
      ...
      if (result) {
        const fileMeanwhileExists = await RNFetchBlob.fs.exists(path); // Check if file was written to path in the meantime by another download.

        if (!fileMeanwhileExists) {
